...
IF (($test).TcpTestSucceeded -eq $True) {
    $arris = [uri]"http://192.168.100.1/cmeventlog.html"
    ...
    IF ($StatusCode -notmatch "40?", "50?") {
        ...
        $url = "http://192.168.100.1/cmeventlog.html"   
        $r = Invoke-WebRequest $url
        ($r.AllElements | Where-Object { $_.class -match 'simpleTable' }).innerText | Out-File -FilePath "$($env:temp)\ArrisEventLog-$($today).txt" 
